...
public class HiredCandidateService implements IHiredCandidateService {
	...
	public List<HiredCandidate> getCandidate(String token) {
		...
		if (isuserpresent) {
			...
			throw new HireCandidateException("user not found");
	}
	...
	public HiredCandidate getCandidateById(String token, long candidateId) {
		boolean isuserpresent = restTemplate.getForObject("http://Lms-admin/user/verify?token=" + token, Boolean.class);
		if (isuserpresent) {
			return hiredCandidateRepository.findById(candidateId).orElseThrow(
					() -> new HireCandidateException("User with id " + candidateId + " does not exist..!"));
		} else
			throw new HireCandidateException("user not found");
	}
